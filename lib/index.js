...
const matching = require('./parser/matching');
...
exports.setCommand = (command) => {
  if (command.signature.parameters) {
    command.signature.parameters = _.map(command.signature.parameters, create(Parameter));
  }
 ...
 */
exports.run = (argv) => {
  const tokens = tokenizer.tokenize(argv, {
    mode: MODES.UNIX
  });

  const combinations = matching.getValidCombinations(tokens);
