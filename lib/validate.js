 ...
 *
 * @example
 * if (validate.isOptionsObjectValid({ foo: 'bar' })) {
 *   console.log('This options object is valid');
 * }
 */
exports.isOptionsObjectValid = (options) => {
...
/**
 * @summary Check if a help string is valid
 * @function
 * @public
 *
 ...
 *
 * @example
 * if (validate.isHelpValid('my help')) {
 *   console.log('This help string is valid');
 * }
 */
exports.isHelpValid = (help) => {
  return exports.isDescriptionValid(help);
};

/**
 * @summary Check if a placeholder is valid
 * @function
 * @public
 *
 ...
 *
 * @example
 * if (validate.isPlaceholderValid('placeholder')) {
 *   console.log('This placeholder is valid');
 * }
 */
exports.isPlaceholderValid = (placeholder) => {
  return _.isNil(placeholder) || isStringValid(placeholder);
};
...
/**
 * @summary Check if a set of examples is valid
 * @function
 * @public
 *
 * @param {Object[]} examples - examples
 * @returns {Boolean}
 *
 * @example
 * if (validate.isExamplesValid([
 *   new Example({
 *     parameters: [ 'bar', 'baz' ],
 ...
 *   })
 * ])) {
 *   console.log('This set of examples is valid');
 * }
 */
exports.isExamplesValid = (examples) => {
  return _.isNil(examples) || isListOfType(examples, Example);
};

/**
 * @summary Check if a set of middlewares is valid
 * @function
 * @public
 *
 ...
 *
 * @example
 * if (validate.isMiddlewaresValid([
 *   (params, options, context) => Promise.resolve(),
 *   (params, options, context) => Promise.resolve(),
 *   (params, options, context) => Promise.resolve()
 * ])) {
 *   console.log('This set of middlewares is valid');
 * }
 */
exports.isMiddlewaresValid = (middlewares) => {
  return _.isNil(middlewares) || isListContentsValid(middlewares, _.isFunction);
};

/**
 * @summary Check if a set of options is valid
 * @function
 * @public
 *
 * @param {Object[]} options - options
 ...
 *   })
 * ])) {
 *   console.log('This set of options is valid');
 * }
 */
exports.isOptionsValid = (options) => {
  const names = _.reduce(options, (accumulator, option) => {
    return _.concat(accumulator, [ option.name ], option.aliases);
  }, []);

  return _.isNil(options) || _.every([
    isListOfType(options, Option),
    isUniqueListValid(options, 'name'),
    _.size(names) === _.size(_.uniq(names))
  ]);
};
...
exports.isAliasesValid = (aliases) => {
  return isListContentsValid(aliases, (alias) => {
    return isStringValid(alias) && exports.isNameValid(alias);
  }) && isUniqueListValid(aliases);
};
