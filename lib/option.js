...
class Option {
  ...
  constructor(options) {
    ...
    // If a boolean option is not passed, and the user didn't set a default value,
    // then we can safely default it to false, for the user's convenience.
    if (Option.isBoolean(options) && options.optional && _.isNil(options.default)) {
...
/**
 * @summary Get the names of an option
 * @function
 * @public
 *
 ...
 * });
 *
 * const names = Option.getNames(option);
 * console.log(names);
 * > [ 'foo', 'bar', 'baz' ]
 */
Option.getNames = (option) => {
  return _.concat([ option.name ], option.aliases);
};
...
/**
 * @summary Compile an option against a value
 * @function
 * @public
 *
 * @param {Object} option - option
 * @param {*} value - value
 * @returns {(Null|Undefined|String|Number|Boolean)} option value
 *
 * @example
 * const value = Option.compile(new Option({
 *   name: 'foo',
 *   type: [ 'number' ]
 * }), '5');
 *
 ...
 * > 5
 */
Option.compile = (option, value) => {
  if (_.isNil(value) || _.every([
    _.includes(option.type, types.supported.STRING),
    !_.isBoolean(value),
    _.isEmpty(value)
  ])) {
    return option.optional ? option.default : null;
  }

  if (_.isBoolean(value)) {
    return _.includes(option.type, types.supported.BOOLEAN) ? value : null;
  }

  return words.evaluate(option.type, value);
};
