...
class Option {
   ...
   * @param {Boolean} [options.inverse=false] - add the inverse option (only when type == boolean)
   *
   * @example
  ...
  constructor(options) {
    ...
    if (!validate.isAliasesValid(options.aliases)) {
    ...
    if (!validate.isFlagValid(options.inverse)) {
      throw new Error(`Invalid option inverse flag: ${options.inverse}`);
    }
    ...
    if (options.inverse && !Option.isBoolean(options)) {
      throw new Error('Invalid option inverse flag: can\'t set when type is boolean');
    }
    ...
    return _.chain(this.name)
      ...
      .thru((options) => {
        return _.join(_.map(options, (option) => {
          return MODES.UNIX.optionToString(option, this);
        }), ', ');
      })
      .thru((option) => {
