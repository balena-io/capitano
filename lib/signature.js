...
Signature.match = (signature, argv) => {
  ...
  if (!_.isEmpty(matches.pairs) && _.size(matches.pairs) < _.size(signature.parameters)) {
    return null;
  }

  if (matches.arguments < _.size(args)) {
    return null;
  }

  const result = {};

  for (const [ words, parameter ] of matches.pairs) {
    if (!Parameter.matches(parameter, words)) {
      return null;
    }

    if (Parameter.isVariadic(parameter)) {
      _.set(result, [ parameter.name ], words);
    } else if (!_.isEmpty(words)) {
      _.set(result, [ parameter.name ], _.first(words));
    }
  }
