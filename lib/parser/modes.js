...
module.exports = {
  ...
  UNIX: {
    ...
    parameterVariadic: /^[[<][a-z,]+\.\.\.[\]>]$/,
    parameterOptional: /^\[.*\]$/,
    endOfParameterSymbol: '--',
    optionToString: (name) => {
      const SHORT_OPTION_MAXIMUM_LENGTH = 1;
      return _.size(name) > SHORT_OPTION_MAXIMUM_LENGTH ? `--${name}` : `-${name}`;
    },
    parameterToString: (name, options) => {
      const CONTENT = options.variadic ? `${name}...` : name;
      return options.optional ? `[${CONTENT}]` : `<${CONTENT}>`;
    }
  }
