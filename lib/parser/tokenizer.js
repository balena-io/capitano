...
const getRelativeToken = (tokens, index, offset) => {
  ...
  // This is a shortcut for optimisation purposes. Even though we allow tokens
  // to be in any order, in practice, tokens are in order, so lets see if the
  // contiguous token is the one we're looking for instead of falling back
  // to more complex find logic.
  const RELATIVE_TOKEN_INDEX = index + offset;
  if (_.get(tokens, [ RELATIVE_TOKEN_INDEX, 'index' ]) === RELATIVE_TOKEN_INDEX) {
    return tokens[RELATIVE_TOKEN_INDEX];
  }
