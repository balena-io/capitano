...
class Parameter {
   ...
   */
  constructor(options) {
    if (!validate.isOptionsValid(options)) {
 ...
 * }
 */
Parameter.matches = (parameter, words) => {
  const VARIADIC_MINIMUM_WORD_SIZE = 2;

  if (_.some([
    !Parameter.isVariadic(parameter) && _.size(words) >= VARIADIC_MINIMUM_WORD_SIZE,
    !Parameter.isOptional(parameter) && _.isEmpty(words)
  ])) {
    return false;
  }

  return _.every(words, (word) => {
    return _.some(parameter.type, (type) => {
