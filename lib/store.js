...
exports.look = (argv, node = nodes.root) => {
  const alternatives = _.reduce(node.children, (accumulator, child, name) => {
    if (_.isEmpty(argv) && wildcard.matches(name, argv)) {
      return _.concat(accumulator, [
        {
          node: child,
          consumed: 0
        }
    ...
    if (wildcard.isVariadic(name) && wildcard.matches(name, argv)) {
      return _.concat(accumulator, [
        {
          node: child,
          consumed: _.size(argv)
        }
      ]);
    }

    const head = _.first(argv);
    if (head === name || wildcard.matches(name, [ head ])) {
      return _.concat(accumulator, [
        {
          node: child,
          consumed: 1
  ...
  if (_.isEmpty(alternatives) && _.isEmpty(argv)) {
    return _.compact([ node.value ]);
  }

  return _.flatMap(alternatives, (alternative) => {
    return exports.look(_.slice(argv, alternative.consumed), alternative.node);
  });
};
